...
def main():
    model = ResNetLike()
    model.load_state_dict(torch.load("best_resnet50_MINST-DVS2.pt", weights_only=True))
    model.eval()
    input = [np.ones(201)]
    input = torch.Tensor(input).unsqueeze(0).to("cuda:0")
    print(input.shape)
    label= np.ones(1000)
    print(torch.max(model.forward(input).data,1))
    _, predicted = torch.max(model.forward(input).data, 1)
    idx = 100
    label[idx-50:idx+51] = predicted.to("cpu")
    print(label)
    pass
